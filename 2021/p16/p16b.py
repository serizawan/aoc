...
class Packet:
    ...
    def __init__(self, version, type_id, length=0):
...
class LiteralPacket(Packet):
    ...
    def __init__(self, version, type_id, length=0, value=None):
    ...
    def parse_value(self, payload: str) -> str:
        ...
        self.length = shift + Packet.VERSION_BIT_LENGTH + Packet.TYPE_ID_BIT_LENGTH
...
class PacketOperator(Packet):
    ...
    def value(self) -> int:
        if self.type_id == 0:
            ...
            return math.prod(sp.value for sp in self.sub_packets)
        elif self.type_id == 2:
    ...
    while bin_repr and int(bin_repr) > 0 and len(packets) != packet_nb and packet_length != packet_len:
        ...
        if packet.__class__ == LiteralPacket:
            ...
            if packet.__class__ == OperatorMode0Packet:
                ...
                current_packet_length = Packet.HEADER_LEN + OperatorMode0Packet.MODE_SP_INFO_LEN
            else:
                sp_version_sum, bin_repr, sub_packets = parse_bin_repr(bin_repr, packet_nb=packet.sub_packets_nb)
                current_packet_length = Packet.HEADER_LEN + OperatorMode1Packet.MODE_SP_INFO_LEN

            current_packet_length += sum([sp.length for sp in sub_packets])
            packet.length = current_packet_length
...
def run() -> None:
    ...
    for version_sum, packet in packets:
