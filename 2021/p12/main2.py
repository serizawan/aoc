...
class CaveMap:
    ...
    def explore(self, cave: Cave, visited: list[Cave], path_count: int, has_joker: bool) -> int:
        if has_joker:
            visitable = cave.neighbors - set([self.caves['start']])
        else:
            visitable = cave.neighbors - set([v for v in visited if not v.is_big])
        if cave.id == 'end':
            return path_count + 1
        elif not visitable:
            return path_count
            ...
            while visitable:
                ...
                if not next_cave.is_big and next_cave in visited:
                    visited.append(next_cave)
                    path_count = self.explore(next_cave, visited, path_count, False)
                else:
                    visited.append(next_cave)
                    path_count = self.explore(next_cave, visited, path_count, has_joker)
                visited.pop()
            return path_count
...
def run() -> None:
    ...
    path_count = cave_map.explore(cave_map.start_cave, [cave_map.start_cave], 0, True)
    print(path_count)
